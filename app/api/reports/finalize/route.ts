...
//  - Logged-in admin (normal session)
export async function POST(request: Request) {
  ...
  // Payments: filter by receipt date (consistent with Resumen)
  ...
  for (const p of payments) {
    ...
    dateByUnit[p.unit_id] = p.date; // last date wins (ordered by date asc)
  }
...
<body>
  ...
  // ── Upsert documents entry (skip if already exists for this month) ───────────
  ...
  if (!existing) {
    ...
    // Update title/timestamp in case of regeneration
